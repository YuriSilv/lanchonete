...
public class JsonConex {
    ...
    public static String pedidosPath = "src\\main\\java\\data\\pedidos.json";
    ...
    public static void dump(Pedidos pedido) throws IOException{
        FileWriter writeFile = null;
        ArrayList<String> dados = open(pedidosPath);
        Gson gson = new Gson();
        String jsonDados = gson.toJson(pedido);
        dados.add(jsonDados);
        Pedidos.setControlID(dados.size());
        try {
            writeFile = new FileWriter(pedidosPath, false);
            writeFile.write(dados.toString());
            writeFile.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static ArrayList<Pedidos> dadosPedidos(){
        File file = new File(pedidosPath);
        Gson gson = new Gson();
        try {
            String content = new String(Files.readAllBytes(Paths.get(file.toURI())));
            ArrayList<Pedidos> dados = gson.fromJson(content, new TypeToken<ArrayList<Pedidos>>() {}.getType());
            return dados;
        } catch (IOException e) {
            e.printStackTrace();
        }
